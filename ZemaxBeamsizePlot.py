...
class BeamProfile:
    def __init__(self,array,xsize,ysize):
        ...
        # Multiply for Near Field Analysis
        self.x_size = xsize
        self.y_size = ysize
        
        # Generate normalized single axis 1D array
        self.X = np.sum(self.A, axis = 0)  # Detector summed along Y axis
        self.Y = np.sum(self.A, axis = 1)  # Detector summed along X axis
        
        # DETECTOR SIZE IN PIXELS
        self.x_len = len(self.X) # Detector size in x (pixels)
        self.y_len = len(self.Y) # Detector size in y (pixels)
        
        # GENERATE X_x and Y_x AXIS IN PIXELS
        self.Xxp = np.linspace(1,self.x_len,self.x_len) # GENERATE X_x AXIS
        self.Yxp = np.linspace(1,self.y_len,self.y_len) # GENERATE Y_x AXIS
        
        # GENERATE KNIFE EDGE DATA
        self.XKE = self.knifeEdge(self.X)
        self.YKE = self.knifeEdge(self.Y)
        
        # FIND CENTER INDEX BASED ON 50% KNIFE EDGE
        self.x_center = self.findPoint(self.XKE,0.5)
        self.y_center = self.findPoint(self.YKE,0.5)
            
        # PIXEL SIZE
        self.x_pixel_size = self.x_size/self.x_len
        self.y_pixel_size = self.y_size/self.y_len
            
        # GENERATE X_x AND Y_x AXIS IN UM    
        self.Xxm = self.Xxp * self.x_pixel_size
        self.Yxm = self.Yxp * self.y_pixel_size
        
        # GET BEAM WIDTHS
        R, a, b = self.findWidth(self.X)
        self.deltax = (b - a) * self.x_pixel_size
        R, a, b = self.findWidth(self.Y)
        self.deltay = (b - a) * self.y_pixel_size
    ...
    def colorPlot(self,title='Color Plot'):
        ...
        plt.xlabel('x (mm)')
        plt.ylabel('y (mm)')
        plt.text(-o_x*0.9,-o_y*0.8,"X Center: {:.4f} mm\nY Center: {:.4f} mm".format(Cx,Cy),color='white')
        
        
        plt.text(-o_x*0.9,o_y*0.6,"X Width: {:.4f} mm\nY Width: {:.4f} mm".format(self.deltax,self.deltay),color='white')
    ...
    def findPoint(self,KE,pmin = 0.5):
        ...
        for j in range(0,l,1):
            if(KE[j] >= pmin):
                #print("{:.0f}% index = {}".format(pmin*100,j))
                a = j
                break
        return a
